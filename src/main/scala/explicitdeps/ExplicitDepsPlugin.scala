...
object ExplicitDepsPlugin                     extends AutoPlugin:
  ...
  private def dependencyFromModule(
    module: sbt.librarymanagement.ModuleID,
    scala: explicitdeps.ScalaVersion
  ): Dependency =
    val binarySuffix = s"_${scala.binary}"
    val fullSuffix   = s"_${scala.full}"
    if module.name.endsWith(binarySuffix) then
      Dependency(module.organization, module.name.stripSuffix(binarySuffix), module.revision, true)
    else if module.name.endsWith(fullSuffix) then
      Dependency(module.organization, module.name.stripSuffix(fullSuffix), module.revision, true)
    else
      Dependency(
        module.organization,
        module.name,
