...
object BoringStuff:
  def jarFileToDependency(scalaVersion: ScalaVersion, log: Logger)(jar: File): Option[Dependency] =
    ...
    recovered.foreach(_ =>
      log.warn(s"Recovered module metadata from cache files for unmatched artifact: ${jar.getAbsolutePath}")
    )
    recovered
  ...
  private def findPomFile(jar: File): Option[File] =
    jar.getName
      ...
      .filter(_.nonEmpty)
      .map(_.mkString("-") + ".pom")
      .map(name => File(jar.getParentFile, name))
      .find(_.exists)
  ...
  private def findIvyFileInIvyCache(jar: File): Option[File] =
    ...
    (parts.tails.toList.reverse ++ parts.inits.toList.tail)
      .filter(_.nonEmpty)
      .map(parts => File(jar.getParentFile.getParentFile, s"ivy-${parts.mkString("-")}.xml"))
      .find(_.exists)
