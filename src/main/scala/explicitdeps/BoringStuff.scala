...
object BoringStuff:
  def jarFileToDependency(scalaVersion: ScalaVersion, log: Logger)(jar: File): Option[Dependency] =
    val recovered = findPomFile(jar).flatMap(parsePomFile(scalaVersion, log))
    ...
    recovered.foreach(_ => log.warn(s"Recovered module metadata from cache files for unmatched artifact: ${jar.getAbsolutePath}"))
    recovered

  private def findPomFile(jar: File): Option[File] =
    jar.getName.dropRight(4).split('-').inits.filter(_.nonEmpty).map(_.mkString("-") + ".pom")
      .map(name => File(jar.getParentFile, name)).find(_.exists)

  private def findIvyFileInIvyCache(jar: File): Option[File] =
    val parts = jar.getName.dropRight(4).split('-').drop(1)
    (parts.tails.toList.reverse ++ parts.inits.toList.tail).filter(_.nonEmpty)
      .map(parts => File(jar.getParentFile.getParentFile, s"ivy-${parts.mkString("-")}.xml")).find(_.exists)

  private def findIvyFileInIvyLocal(jar: File): Option[File] =
    Some(File(jar.getParentFile.getParentFile, "ivys/ivy.xml")).filter(_.exists)
  ...
  private def parsePomFile(scalaVersion: ScalaVersion, log: Logger)(file: File): Option[Dependency] =
    ...
    catch case NonFatal(_) => log.warn(s"Failed to parse dependency information from POM file ${file.getAbsolutePath}"); None
  ...
  private def parseIvyFile(scalaVersion: ScalaVersion, log: Logger)(file: File): Option[Dependency] =
    ...
    catch case NonFatal(_) => log.warn(s"Failed to parse dependency information from Ivy file ${file.getAbsolutePath}"); None

  private def parseModuleName(scalaVersion: ScalaVersion)(raw: String): (String, Boolean) =
    if raw.endsWith(s"_${scalaVersion.binary}") then (raw.stripSuffix(s"_${scalaVersion.binary}"), true)
    else if raw.endsWith(s"_${scalaVersion.full}") then (raw.stripSuffix(s"_${scalaVersion.full}"), true)
    else (raw, false)
